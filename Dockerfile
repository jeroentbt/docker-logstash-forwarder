...
RUN export GOVERSION="1.5.1.linux-amd64" && \
    ...
    curl -sL https://storage.googleapis.com/golang/go${GOVERSION}.tar.gz > go.tar.gz && \
...
RUN export LSFVERSION="0.4.0" && \
    ...
    curl -sL https://github.com/elastic/logstash-forwarder/archive/v${LSFVERSION}.tar.gz > lsf.tar.gz && \
...
CMD go version && echo "logstash-forwarder version" $(logstash-forwarder --version)
